...
module Heartwood
  class IconSpriteGenerator < Rails::Generators::Base

    desc 'Combine all SVG files from app/assets/images/icons to app/assets/images/icons.svg'

    source_root File.expand_path('../../templates', __FILE__)

    def locate_icons
      icons_dir = Rails.root.join('app', 'assets', 'images', 'icons')
      @icon_files = Dir.glob("#{icons_dir}/*.svg")
    end

    def process_icons
      @icons_html = ''
      @icon_files.each do |icon|
        doc = Nokogiri::XML(File.read(icon))
        @icon_name = doc.css('#Page-1 > g')[0]['id']
        @icon_html = doc.css('#Page-1 g').to_s.gsub(/[a-z\-]+=\"[a-zA-Z0-9\-\#]+\"/, '')
        @icons_html << template_snippet('icon.svg')
      end
    end

    def render_result
      file = 'app/assets/images/icons.svg'
      template "#{file}.erb", file, force: true
    end
